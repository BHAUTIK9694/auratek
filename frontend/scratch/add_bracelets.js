...
const adjectives = ['Eternity', 'Radiant', 'Classic', 'Signature', 'Infinity', 'Halo', 'Royal', 'Celestial'];
const materials = ['Diamond', 'Rose Gold', 'White Gold', 'Yellow Gold'];
const types = ['Tennis Bracelet', 'Bangle', 'Link Bracelet', 'Cuff'];
...
const newItems = files.map((file, i) => {
  ...
  const newFilename = `bracelet-collection-${i + 1}${ext}`;
  ...
  return {
    id: `bracelet-${i + 1}`,
    name: name,
    category: `Bracelets · ${materialCat}`,
...
if (newArrivalsEndIndex !== -1) {
  ...
  console.log(`Successfully added ${newItems.length} bracelets to data.js`);
} else {
