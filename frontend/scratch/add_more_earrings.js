...
const adjectives = ['Exquisite', 'Ornate', 'Sparkling', 'Dazzling', 'Opulent', 'Brilliant', 'Statement', 'Luxury'];
const materials = ['Diamond', 'Rose Gold', 'White Gold', 'Yellow Gold'];
const types = ['Drop Earrings', 'Chandelier Earrings', 'Studs', 'Hoops'];
...
const newItems = files.map((file, i) => {
  ...
  const newFilename = `earring-collection-v2-${i + 1}${ext}`;
...
if (newArrivalsEndIndex !== -1) {
  ...
  console.log(`Successfully added ${newItems.length} more earrings to data.js`);
} else {
