...
const adjectives = ['Exquisite', 'Ornate', 'Sparkling', 'Dazzling', 'Opulent', 'Brilliant', 'Statement', 'Luxury', 'Classic', 'Eternity'];
const materials = ['Diamond', 'Rose Gold', 'White Gold', 'Yellow Gold'];
const types = ['Necklace', 'Choker', 'Collar', 'Pendant Necklace', 'Layered Necklace'];
...
const newItems = files.map((file, i) => {
  ...
  const newFilename = `necklace-collection-v2-${i + 1}${ext}`;
  ...
  return {
    id: `necklace-v2-${i + 1}`,
    name: name,
    category: `Necklaces · ${materialCat}`,
...
if (newArrivalsEndIndex !== -1) {
  ...
  console.log(`Successfully added ${newItems.length} more necklaces to data.js`);
} else {
