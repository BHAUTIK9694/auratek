...
const adjectives = ['Signature', 'Ornate', 'Classic', 'Radiant', 'Intricate', 'Timeless', 'Luminous', 'Majestic', 'Regal', 'Delicate'];
const materials = ['Diamond', 'Rose Gold', 'White Gold', 'Yellow Gold', 'Pearl', 'Emerald', 'Ruby', 'Sapphire'];
const types = ['Pendant', 'Motif Pendant', 'Drop Pendant', 'Medallion'];
...
const newItems = files.map((file, i) => {
  ...
  const newFilename = `pendant-collection-${i + 1}${ext}`;
  ...
  const materialCat = getRandomItem(['18k Rose Gold', '18k White Gold', '18k Yellow Gold']);
  
  return {
    id: `pendant-${i + 1}`,
    name: name,
    category: `Pendants · ${materialCat}`,
...
if (newArrivalsEndIndex !== -1) {
  ...
  console.log(`Successfully added ${newItems.length} pendants to data.js`);
} else {
